...
async function acceptTradeRequest(tradeRequestId, io = null) {
  return await TradeRepository.acceptTradeRequest(tradeRequestId, io);
}
...
async function rejectTradeRequest(tradeRequestId, ownerId, io = null) {
  ...
  try {
    ...
    if (photoCard?.gradeId) {
      const grade = await prisma.cardGrade.findUnique({ where: { id: photoCard.gradeId }, select: { name: true } });
      gradeName = grade?.name ? `[${grade.name}] ` : '';
    }
    const applicantId = tradeRequest.applicantId;
    const ownerName = owner?.nickname || '판매자';
    const cardName = photoCard?.name || '포토카드';
    const message = `${ownerName}님과의 [${gradeName} | ${cardName}] 포토카드 교환이 불발되었습니다.`;
